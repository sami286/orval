...
export const getAuctionDetails = (auctionId: string) =>
  instance<AuctionDetails>({ url: `/auctions/${auctionId}`, method: 'GET' })
...
Current limitations: dates inside `oneOf`/`anyOf` variants and
`additionalProperties` maps are not converted, responses with multiple
success shapes are skipped, and only the axios-based clients are wired
(`react-query`, `vue-query`, `svelte-query` with `httpClient: 'axios'`).
